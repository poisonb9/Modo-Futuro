...
def teste_positivo_manda_recorte_quando_existe():
    assert 'entradas["recorte"] = item["recorte"]' in FONTE
...
def teste_negativo_NAO_manda_recorte_quando_nao_ha():
    ...
    """
    i = FONTE.index('entradas["recorte"]')
    antes = FONTE[:i].rstrip().split("\n")[-1]
    assert antes.strip().startswith("if item.get(\"recorte\")"), \
        f"recorte nao esta' sob condicional: ...{antes}"
    assert '"recorte":' not in BLOCO, \
        "recorte esta' no dicionario fixo — vai junto mesmo quando vazio"
