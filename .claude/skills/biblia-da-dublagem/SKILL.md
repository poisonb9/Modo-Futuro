...
- **O motor**, em `engine/biblia.py`:
  - `## Tom`, `## Glossário` e `## Proibido` entram no prompt da narração
    (`engine/traducao.py`, campo `{biblia}`).
...
- Mudou algo? Rodar `python teste/teste_biblia.py`.
